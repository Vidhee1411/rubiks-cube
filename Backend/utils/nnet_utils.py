...
def make_batches(states_nnet: List[np.ndarray], ctgs: np.ndarray, actions: np.array,
                 batch_size: int) -> List[Tuple[List[np.ndarray], np.ndarray, np.array]]:
    ...
    while (start_idx + batch_size) <= num_examples:
        ...
        batches.append((inputs_batch, ctgs_batch, actions_batch))
...
def train_nnet(nnet: nn.Module, states_nnet: List[np.ndarray], ctgs: np.ndarray,
               actions: List[int], device: torch.device, batch_size: int, num_itrs: int, train_itr: int,
               lr: float, lr_d: float, display: bool = True) -> float:
    # optimization
    display_itrs = 100
    ...
    # train network
    batches: List[Tuple[List, np.ndarray, List[int]]] = make_batches(states_nnet, ctgs, np.array(actions), batch_size)
    ...
    while train_itr < max_itrs:
        ...
        # get data
        inputs_batch, ctgs_batch_np, actions_batch_np = batches[batch_idx]
        ...
        # forward
        actions_batch = torch.tensor(np.expand_dims(actions_batch_np, 1), device=device)
        # TODO assuming one tensor per list and can be concatenated
        # states_cat = torch.cat((states_batch[0], states_goal_batch[0]), dim=1)
        ctgs_nnet: Tensor = nnet(*[states_batch[0]]).gather(1, actions_batch)[:, 0]
...
# heuristic
def get_heuristic_fn_old(nnet: nn.Module, device: torch.device, env: Environment, clip_zero: bool = False,
    ...
    def heuristic_fn(states: List, is_nnet_format: bool = False) -> np.ndarray:
        ...
        while start_idx < num_states:
            ...
            # TODO assuming one array and can be concatenated
            # states_nnet_batch_tensors_cat = torch.cat((states_nnet_batch_tensors[0],
            #                                           states_goal_nnet_batch_tensors[0]), dim=1)
            cost_to_go_batch: np.ndarray = nnet(*[states_nnet_batch_tensors[0]]).cpu().data.numpy()
...
def get_heuristic_fn(nnet: nn.Module, device: torch.device, env: Environment, clip_zero: bool = False,
    ...
    def heuristic_fn(states: List, states_goal: List, is_nnet_format: bool = False) -> np.ndarray:
        cost_to_go_l: List[np.ndarray] = []
        ...
        while start_idx < num_states:
            ...
            if not is_nnet_format:
                ...
                states_goal_batch: List = states_goal[start_idx:end_idx]
                states_goal_nnet_batch: List[np.ndarray] = env.state_to_nnet_input(states_goal_batch)
            else:
                states_nnet_batch = [x[start_idx:end_idx] for x in states]
                states_goal_nnet_batch = [x[start_idx:end_idx] for x in states_goal]

            # get nnet output
            states_nnet_batch_tensors = states_nnet_to_pytorch_input(states_nnet_batch, device)
            states_goal_nnet_batch_tensors = states_nnet_to_pytorch_input(states_goal_nnet_batch, device)

            # TODO assuming one array and can be concatenated
            # states_nnet_batch_tensors_cat = torch.cat((states_nnet_batch_tensors[0],
            #                                           states_goal_nnet_batch_tensors[0]), dim=1)
            cost_to_go_batch: np.ndarray = nnet(*[states_nnet_batch_tensors[0],
                                                  states_goal_nnet_batch_tensors[0]]).cpu().data.numpy()
            cost_to_go_l.append(cost_to_go_batch[:, 0])
...
def load_heuristic_fn(nnet_dir: str, device: torch.device, on_gpu: bool, nnet: nn.Module, env: Environment,
                      clip_zero: bool = False, gpu_num: Optional[int] = None, batch_size: Optional[int] = None):
...
def model_fn_queue(input_queue, output_queue, proc_id):
    def model_fn(states: np.ndarray, actions: np.ndarray):
        input_queue.put((proc_id, states, actions))
        states_next = output_queue.get()
...
def heuristic_fn_runner(heuristic_fn_input_queue: Queue, heuristic_fn_output_queues, nnet_dir: str,
                        device, on_gpu: bool, gpu_num: Optional[int], env: Environment, all_zeros: bool,
                        clip_zero: bool, batch_size: Optional[int]):
    heuristic_fn = None
    if not all_zeros:
        heuristic_fn = load_heuristic_fn(nnet_dir, device, on_gpu, env.get_nnet(), env, gpu_num=gpu_num,
...
def stop_model_fn_runners(procs, input_queue):
    for _ in procs:
