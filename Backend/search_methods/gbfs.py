...
import numpy as np
from utils import search_utils, misc_utils
...
class Instance:
    ...
    def __init__(self, state: State, eps: float):
        ...
        self.trajs: List[Tuple[State, int, float]] = []
        self.seen_states: Set[State] = set()

        self.eps = eps

    def add_to_traj(self, state: State, action: int, cost_to_go: float):
        self.trajs.append((state, action, cost_to_go))
...
class GBFS:
    ...
    def get_trajs(self) -> List[List[Tuple[State, int, float]]]:
        trajs_all: List[List[Tuple[State, int, float]]] = []
    ...
    def _record_solved(self) -> None:
        ...
        if len(solved_idxs) > 0:
            ...
            rand_actions: List[int] = self.env.rand_action(states_solved)

            for instance, state, action in zip(instances_solved, states_solved, rand_actions):
                instance.add_to_traj(state, action, 0.0)
    ...
    def _move(self, heuristic_fn: Callable) -> None:
        ...
        for idx in range(len(instances)):
            seen_state: bool = states[idx] in instances[idx].seen_states
            if seen_state:
                eps_l.append(1.0)
            else:
                eps_l.append(instances[idx].eps)

        ctg_backups: np.array
        actions: List[int]
        states_next: List[State]
        ctg_backups, actions, states_next = search_utils.q_step(states, heuristic_fn, self.env, eps_l)
        ...
        for idx in range(len(instances)):
            ...
            instance.add_to_traj(state, action, ctg_backup)

            # get next state
            state_next: State = states_next[idx]
...
def gbfs_test(num_states: int, back_max: int, env: Environment, heuristic_fn: Callable,
              max_solve_steps: Optional[int] = None) -> float:
    # get data
    ...
    per_solved_all: float = 100 * float(sum(is_solved_all)) / float(len(is_solved_all))

    # Get state cost-to-go
    state_ctg_all: np.ndarray = heuristic_fn(states).min(axis=1)

    print("GBFS solved: %f" % per_solved_all)
